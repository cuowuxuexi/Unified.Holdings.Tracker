...
published:
created: 2025-11-15
description: "GPT-5.1, our newest flagship model, is designed to balance intelligence and speed for a variety of agentic and coding tasks, while also i..."
tags:
  - "clippings"
---
...
### Quick reference for migrating prompts

| Scenario | Recommendation | Rationale |
| --- | --- | --- |
| GPT-4.1 → GPT-5.1 (`none`) | Start by reusing the prompt verbatim, then tighten instructions only if replies feel too terse. Keep `reasoning.effort = "none"` unless the workload truly needs long-form reasoning. | GPT-5.1 already balances brevity and accuracy for easy turns; over-specifying up front can make responses stiff.
| GPT-5 → GPT-5.1 (`auto` / default) | Add an explicit persistence clause plus a reflection step after tool calls. Monitor token use and trim redundant scaffolding once behavior stabilizes. | GPT-5.1 is far more token-efficient and may cut tail work without a gentle nudge to “finish the loop.”
| Structured-output or evaluator pipelines | Combine `response_format` (JSON or XML) with short bullet rules that restate required fields. | GPT-5.1 follows schemas exceptionally well when the contract is plain-language and deduplicated.
| High-latency or streaming experiences | Use `none` reasoning, cap max output tokens in the system prompt, and defer deep dives to user opt-in. | Keeps turn time predictable while still letting the agent escalate detail when explicitly asked.

When drafting brand new prompts, keep each of the following blocks short (2–4 sentences) so requirements stay debuggable:

1. **Persona & mission** – What the agent optimizes for and where it should gracefully say “out of scope.”
2. **Interaction principles** – Tone, pacing, and when to acknowledge the user.
3. **Workflow expectations** – How to plan, how to verify, and what “done” means.
4. **Tool contract** – Eligibility checks, argument validation, and post-call summaries.
5. **Output contract** – Formatting, verbosity limits, schemas, or XML/JSON wrappers.

Explicit section headers (or XML tags) dramatically reduce conflicting instructions and make later audits easier.
...
If you need a neutral starting point, scaffold your system prompt with consistent sections so later debugging targets the correct clause. A lightweight template we use internally looks like this:

```
<persona>
  Who you help, how success is measured, and which requests to decline.
</persona>

<interaction_principles>
  Tone, acknowledgement policy, pacing rules for short vs. long turns.
</interaction_principles>

<workflow>
  Planning rules, clarifying-question thresholds, verification requirements, and done criteria.
</workflow>

<tool_contract>
  How to choose tools, validate inputs, and summarize outputs.
</tool_contract>

<output_contract>
  Formatting, schema, verbosity ceilings/floors, and citation expectations.
</output_contract>
```

Separating these sections keeps edits surgical. You can metaprompt or diff only the `<workflow>` block, for example, without accidentally changing tone or formatting requirements elsewhere.
...
### Prompt debugging workflow checklist

1. **Instrument before editing.** Capture every assistant turn with: prompt hash, tool calls, latency, token counts, and final outcome. Without this telemetry, it is impossible to know whether a regression is due to the model or an environment/tool issue.
2. **Bucket failures.** Sample 10–20 bad traces per bucket (e.g., premature stop, overlong answer, wrong tone). Annotate each with “expected vs actual.” These become the seed cases you feed into the metaprompting loops shown above.
3. **Localize the culprit section.** Ask GPT-5.1 (or another trusted reviewer) to map each failure back to a specific prompt block. Edit only that block, re-run the failing samples, and write down the rationale—future you will thank you.
4. **Run a sanity sweep.** After every prompt edit, replay at least one “happy path” scenario in staging plus the failing set. Log deltas in reasoning tokens, runtime, and tool volume so you can catch secondary effects early.
5. **Version and diff.** Store prompts next to code with semantic versioning. Each change should include a short changelog plus the tests you ran so other teams can safely roll back.

### Evaluation, observability, and guardrails

- **Golden suites:** Maintain a small JSON/CSV dataset of canonical user turns (successes, boundary cases, adversarial phrasing). Automate replays daily and gate prompt changes on pass rates.
- **Live eval taps:** Sample ≤1% of production turns, mask PII, and re-run them offline with the candidate prompt/model. Compare acceptance criteria (latency, token usage, final quality tags) before promoting.
- **Structured logging:** Emit metrics for `reasoning_tokens`, `tool_calls`, `plan_updates`, and “stop reason.” Alert when they drift beyond control limits—these are usually early warnings for prompt regressions.
- **Guardrail prompts:** Layer a lightweight safety pre-check (e.g., “refuse finance advice outside scope”) ahead of the main agent to catch obvious violations without polluting the main instructions.
- **Human-in-the-loop hooks:** For high-risk actions (deployments, bookings, trades), require the agent to emit a confirmation summary that a reviewer must approve. Prompting alone is rarely sufficient for irreversible work.
