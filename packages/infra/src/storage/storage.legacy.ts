...
import { getExchangeRateForAssetToCNY } from '../providers/currency-service'; // Import currency service
import { dataService } from '../data/data-service'; // 导入数据访问服务
import {
  Portfolio,
  Transaction,
  TransactionType,
  LeverageInfo,
} from '@uht/domain'; // 确保类型在 ../types 中正确定义
...
export async function readPortfolios(): Promise<Portfolio[]> {
  try {
    ...
    const portfolios = dataService.readJsonFile<Portfolio[]>(
      PORTFOLIOS_FILE,
      []
    );
...
export async function addPortfolio(newPortfolioData: {
  ...
  const newPortfolio: Portfolio = {
    ...
    leverage: newPortfolioData.leverage || {
      totalAmount: 0,
      usedAmount: 0,
      availableAmount: 0,
      costRate: 0,
...
export async function addTransactionToPortfolio(
  ...
  const amount =
    ...
    (transactionData.quantity ?? 0) * (transactionData.price ?? 0);
  ...
  switch (transactionData.type) {
    case TransactionType.BUY: {
      ...
      if (transactionData.assetCode) {
        // --- 增加日志：获取汇率 ---
        console.log(
          `[storage.ts BUY] Asset code found: ${transactionData.assetCode}, checking for exchange rate.`
        );
        // --- 结束日志 ---
        // 简单的判断，实际项目中 getMarketFromCode 应该更健壮
        const isForeign =
          transactionData.assetCode.toLowerCase().startsWith('hk') ||
          transactionData.assetCode.toLowerCase().startsWith('us');
        if (isForeign) {
          try {
            ...
            // --- 增加日志：获取汇率结果 ---
            console.log(
              `[storage.ts BUY] Fetched exchange rate for ${transactionData.assetCode}: ${exchangeRate}`
            );
            // --- 结束日志 ---
          } catch (error) {
            console.error(
              `[storage.ts BUY] Failed to get exchange rate for ${transactionData.assetCode}:`,
              error
            );
      ...
      // --- 增加日志：CNY 金额 ---
      console.log(
        `[storage.ts BUY] Original Cost: ${originalTotalCost}, Rate: ${exchangeRate}, Total Cost CNY: ${totalCostCNY}`
      );
      console.log(
        `[storage.ts BUY] Commission CNY: ${commissionCNY}, Original Leverage Used: ${originalLeverageUsed}, Leverage Used CNY: ${leverageUsedCNY}`
      );
      // --- 结束日志 ---
      ...
      if (portfolio.cash >= commissionCNY) {
        ...
        console.log(
          `[storage.ts BUY] Commission paid with cash. Cash after commission: ${portfolio.cash.toFixed(2)}`
        );
      } else {
        // 现金不足，检查融资额度
        ...
        if (portfolio.leverage.availableAmount >= feeShortfall) {
          ...
          console.log(
            `[storage.ts BUY] Commission partially paid with cash (${(commissionCNY - feeShortfall).toFixed(2)}) and leverage (${feeShortfall.toFixed(2)})`
          );
        } else {
          // 现金和融资都不足以支付手续费
          console.error(
      ...
      if (useLeverage) {
        ...
        console.log(
          `[storage.ts BUY] Need Cash (CNY): ${needCash.toFixed(2)} (TotalCNY: ${totalCostCNY.toFixed(2)} - LeverageCNY: ${leverageUsedCNY.toFixed(2)})`
        );

        if (needCash < -epsilon) {
          console.error(
            `计算错误：需要支付的现金部分远小于零 (needCash: ${needCash.toFixed(8)}). 这可能发生在 leverageUsed > totalCost 时。`
          );
          return null;
        }
        ...
        if (portfolio.cash >= totalCostCNY) {
          ...
          console.log(
            `[storage.ts BUY] Trade cost paid with cash. Cash after: ${portfolio.cash.toFixed(2)}`
          );
        } else {
          // 现金不足，尝试使用融资
          ...
          if (portfolio.leverage.availableAmount >= neededLeverageCNY) {
            ...
            portfolio.cash = 0;
            console.log(
              `[storage.ts BUY] Trade cost covered by cash (${portfolio.cash.toFixed(2)}) and leverage (${neededLeverageCNY.toFixed(2)})`
            );
          } else {
    ...
    case TransactionType.SELL: {
      ...
      if (transactionData.assetCode) {
        console.log(
          `[storage.legacy SELL] Asset code found: ${transactionData.assetCode}, checking for exchange rate.`
        );
        const isForeign =
          transactionData.assetCode.toLowerCase().startsWith('hk') ||
          transactionData.assetCode.toLowerCase().startsWith('us');
        if (isForeign) {
          try {
            ...
            console.log(
              `[storage.legacy SELL] Fetched exchange rate for ${transactionData.assetCode}: ${exchangeRate}`
            );
          } catch (error) {
            console.error(
              `[storage.legacy SELL] Failed to get exchange rate for ${transactionData.assetCode}:`,
              error
            );
            return null;
          }
      ...
      const commissionCNY = (transactionData.commission ?? 0) * exchangeRate;
      const netAmountCNY = amountCNY - commissionCNY;

      console.log(
        `[storage.legacy SELL] Original Amount: ${amount}, Rate: ${exchangeRate}, Amount CNY: ${amountCNY.toFixed(2)}`
      );
      console.log(
        `[storage.legacy SELL] Commission CNY: ${commissionCNY.toFixed(2)}, Net Amount CNY: ${netAmountCNY.toFixed(2)}`
      );
    ...
    case TransactionType.DIVIDEND:
      ...
      console.log(
        `[storage.ts DIVIDEND] Received dividend amount: ${amount}. Adding to cash.`
      );
      portfolio.cash += amount;
      break;
...
export async function deleteTransactionFromPortfolio(
  ...
  if (transactionIndex === -1) {
    console.error(
      `Transaction with ID ${transactionId} not found in portfolio ${portfolioId}.`
    );
    return false;
  }
  ...
  switch (transactionToDelete.type) {
      ...
      if (transactionToDelete.assetCode) {
        ...
        if (isForeign) {
          try {
            ...
            console.error(
              `[storage.legacy Delete SELL] Failed to get exchange rate for ${transactionToDelete.assetCode}:`,
              error
            );
    ...
    case TransactionType.DIVIDEND:
      ...
      if (portfolio.cash >= amount) {
        ...
        console.error(
          `[storage.ts Delete DIVIDEND] Reversing dividend ${transactionId} for portfolio ${portfolioId} would result in negative cash. Current cash: ${portfolio.cash}, Dividend amount: ${amount}. Proceeding, but data may be inconsistent.`
        );
...
export async function cashRecalculateForPortfolioAsync(
  ...
  let availableLeverageCNY =
    portfolio.leverage?.availableAmount ?? portfolio.leverage?.totalAmount ?? 0;
  const steps: Array<{
    txId: string;
    type: string;
    before: number;
  ...
  console.log(
    `[RecalculateAsync] Starting recalculation for portfolio ${portfolio.id}. Initial cash: ${cash}`
  );
  for (const tx of portfolio.transactions) {
    ...
    const isForeign =
      tx.assetCode &&
      (tx.assetCode.toLowerCase().startsWith('hk') ||
        tx.assetCode.toLowerCase().startsWith('us'));
    ...
    if (isForeign && tx.assetCode) {
      try {
        ...
        console.error(
          `[RecalculateAsync] Failed to get exchange rate for ${tx.assetCode} on tx ${tx.id}. Skipping transaction. Error:`,
          error
        );
        continue; // 跳过此交易的处理
      }
    ...
    const amountCNY =
      tx.amount ?? originalQuantity * originalPrice * exchangeRate;
    ...
    const originalLeverageUsed =
      tx.leverageUsed !== undefined
        ? tx.leverageUsed
        : (tx as any).useLeverageAmount !== undefined
          ? (tx as any).useLeverageAmount
          : 0;
    const leverageUsedCNY = originalLeverageUsed * exchangeRate;

    console.log(
      `[RecalculateAsync] Before Tx ${tx.id} (${tx.type}): cash=${before.toFixed(2)}, AmountCNY=${amountCNY.toFixed(2)}, CommissionCNY=${commissionCNY.toFixed(2)}, assetCode=${tx.assetCode}, Rate=${exchangeRate}, OrigLeverage=${originalLeverageUsed}`
    );
    ...
    switch (tx.type) {
      case TransactionType.BUY: {
        ...
        if (useLeverage) {
          ...
          if (needCash < -epsilon) {
            console.warn(
              `[RecalculateAsync BUY Leveraged Tx ${tx.id}] needCash is significantly negative (${needCash.toFixed(8)}). Skipping.`
            );
            continue;
          }
          if (needCash > cash + epsilon) {
            console.warn(
              `[RecalculateAsync BUY Leveraged Tx ${tx.id}] Insufficient cash (Need ${needCash.toFixed(2)}, Have ${cash.toFixed(2)}). Skipping.`
            );
            continue;
          }
          if (leverageUsedCNY > availableLeverageCNY + epsilon) {
            console.warn(
              `[RecalculateAsync BUY Leveraged Tx ${tx.id}] Insufficient available leverage (Need ${leverageUsedCNY.toFixed(2)}, Have ${availableLeverageCNY.toFixed(2)}). Skipping.`
            );
            continue;
          }
          ...
          if (cash >= totalCostCNY - epsilon) {
            ...
            if (availableLeverageCNY >= neededLeverageCNY - epsilon) {
              ...
              console.warn(
                `[RecalculateAsync BUY CashFirst Tx ${tx.id}] Insufficient funds and leverage. Skipping.`
              );
              continue;
            }
          }
      ...
      case TransactionType.WITHDRAW:
        ...
        if (cash >= amountCNY - epsilon) {
          ...
          console.warn(
            `[RecalculateAsync WITHDRAW Tx ${tx.id}] Insufficient cash (${cash.toFixed(2)} < ${amountCNY.toFixed(2)}). Skipping.`
          );
          continue;
        }
        break;
      ...
      case TransactionType.LEVERAGE_REMOVE:
        ...
        if (availableLeverageCNY >= amountCNY - epsilon) {
          ...
          console.warn(
            `[RecalculateAsync LEVERAGE_REMOVE Tx ${tx.id}] Insufficient available leverage (${availableLeverageCNY.toFixed(2)} < ${amountCNY.toFixed(2)}). Skipping.`
          );
          continue;
        }
        break;
      case TransactionType.LEVERAGE_COST:
        ...
        if (cash >= amountCNY - epsilon) {
          ...
          console.warn(
            `[RecalculateAsync LEVERAGE_COST Tx ${tx.id}] Insufficient cash (${cash.toFixed(2)} < ${amountCNY.toFixed(2)}). Skipping.`
          );
          continue;
        }
        break;
    ...
    console.log(
      `[RecalculateAsync] After Tx ${tx.id} (${tx.type}): cash=${cash.toFixed(2)}, usedLeverageCNY=${usedLeverageCNY.toFixed(2)}, availableLeverageCNY=${availableLeverageCNY.toFixed(2)}`
    );
    steps.push({
      txId: tx.id,
      type: tx.type,
  ...
  console.log(
    `[RecalculateAsync] Final recalculated cash for portfolio ${portfolio.id}: ${cash.toFixed(2)}`
  );
...
export async function correctHistoricalTransactionAmounts(): Promise<void> {
  console.log(
    '[Correction] Starting historical transaction amount correction...'
  );
  ...
  try {
    ...
    if (!portfolios || portfolios.length === 0) {
      console.log(
        '[Correction] No portfolios found or data file is empty. Exiting correction.'
      );
      return;
    }
  } catch (error) {
    console.error(
      '[Correction] Error reading portfolios data during correction:',
      error
    );
    return;
  }
  ...
  for (const portfolio of portfolios) {
    ...
    console.log(
      `[Correction] Processing portfolio: ${portfolio.name} (${portfolio.id})`
    );

    // --- 修改点：直接调用异步重算函数 ---
    console.log(
      `[Correction] Recalculating cash for portfolio ${portfolio.id} using async recalculator.`
    );
    try {
      ...
      if (Math.abs(portfolio.cash - recalculatedCash) > epsilon) {
        console.log(
          `[Correction] Portfolio ${portfolio.id} cash requires update. Current: ${portfolio.cash.toFixed(2)}, Recalculated: ${recalculatedCash.toFixed(2)}`
        );
        ...
        console.log(
          `[Correction] Portfolio ${portfolio.id} cash is consistent. Current: ${portfolio.cash.toFixed(2)}`
        );
      }
    } catch (recalcError) {
      console.error(
        `[Correction] Error recalculating cash for portfolio ${portfolio.id}:`,
        recalcError
      );
      //可以选择跳过此投资组合或停止整个过程
      continue; // 跳过此投资组合
  ...
  if (changesMade) {
    ...
    await writePortfolios(portfolios);
    console.log(
      '[Correction] Historical data correction and recalculation completed.'
    );
  } else {
    console.log(
      '[Correction] No portfolio data needed correction or update based on recalculation.'
    );
  }
}
