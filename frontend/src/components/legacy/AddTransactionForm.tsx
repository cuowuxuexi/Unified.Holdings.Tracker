...
import useAppStore from '../../store'; // Adjusted path for legacy directory
import { TransactionType, TransactionInput } from '../../store/types'; // Adjusted path for legacy directory
...
const typeFieldConfig: Record<
  TransactionType,
  {
    requiresAsset?: boolean;
    requiresQuantityPrice?: boolean;
...
const AddTransactionForm: React.FC<AddTransactionFormProps> = ({
  ...
  const leverageData: LegacyLeverageData | undefined =
    portfolio?.leverageInfo ?? portfolio?.leverage;
  const availableLeverage =
    leverageData?.availableCredit ?? leverageData?.availableAmount ?? 0;
  ...
  const onFinish = async (values: TransactionFormValues) => {
    ...
    const baseTransactionData: Partial<
      TransactionInput & { leverageUsed?: number }
    > = {
    ...
    if (
      values.type === TransactionType.BUY ||
      values.type === TransactionType.SELL
    ) {
      if (!values.assetCode) {
        messageApi.error('请输入资产代码！');
      ...
      if (values.type === TransactionType.BUY) {
        ...
        const requiredFunding =
          totalCost - (baseTransactionData.leverageUsed ?? 0);
        if (requiredFunding > availableCash + availableLeverage + epsilon) {
          messageApi.error(
            `资金和融资额度都不足。需要 ${requiredFunding.toFixed(2)}，现金+融资可用 ${(availableCash + availableLeverage).toFixed(2)}。`
          );
          setIsSubmitting(false);
          return;
        }
      ...
      values.type === TransactionType.DEPOSIT ||
      values.type === TransactionType.WITHDRAW ||
      values.type === TransactionType.DIVIDEND
    ) {
    ...
    const transactionData = baseTransactionData as TransactionInput & {
      leverageUsed?: number;
    };
    ...
    */
    ...
    try {
      ...
      messageApi.error(backendMessage || '添加交易记录失败，请重试。');
    } finally {
      setIsSubmitting(false);
    }
  };

  const onFinishFailed: FormProps<TransactionFormValues>['onFinishFailed'] = (
    errorInfo
  ) => {
      ...
      >
        ...
        {config.requiresAsset && (
          <Form.Item
            ...
            rules={[
              {
                required: selectedType !== TransactionType.DIVIDEND,
                message: '请输入资产代码！',
              },
            ]}
        ...
        {config.requiresQuantityPrice && (
          <Form.Item
            ...
            rules={[
              {
                required: config.requiresQuantityPrice,
                message: '请输入数量！',
              },
            ]} // Required only for BUY/SELL
          >
            <InputNumber
              min={0}
              style={{ width: '100%' }}
        ...
        {selectedType === TransactionType.BUY && (
          <Form.Item
            label="本次融资金额"
            help={`可用融资额度：${availableLeverage}，买入总额：${buyTotal}`}
          >
        ...
        {[TransactionType.BUY, TransactionType.SELL].includes(
        ...
        <Form.Item>
          <Button
            ...
            disabled={!portfolioId || !selectedType}
          >
            添加交易记录
          </Button>
        </Form.Item>
