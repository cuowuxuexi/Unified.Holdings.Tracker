...
const KLINE_CACHE_TTL = 15 * 60 * 1000; // 15分钟缓存
  ...
  try {
    ...
    if (market === 'sh' || market === 'sz') {
      ...
      if (dataParts.length >= 48) {
        ...
        console.warn(
          `[parseQuoteLine] Insufficient data fields for A-share quote: ${codeWithPrefix}`
        );
        return null;
      }
    } else if (market === 'hk') {
      ...
      if (dataParts.length >= 46) {
        ...
        if (timeStr && timeStr.includes('/') && timeStr.includes(':')) {
          ...
          try {
            ...
            if (!isNaN(parsedTimestamp)) {
              ...
              console.warn(
                `[parseQuoteLine] Failed to parse HK timestamp (NaN): ${timeStr}`
              );
            }
          } catch (e) {
            console.warn(
              `[parseQuoteLine] Error parsing HK timestamp: ${timeStr}`,
              e
            );
            // 保留默认的 Date.now()
          }
        ...
        console.warn(
          `[parseQuoteLine] Insufficient data fields for HK-share quote: ${codeWithPrefix}`
        );
        return null;
      }
    } else if (market === 'us') {
      ...
      if (dataParts.length >= 46) {
        ...
        if (timeStr && timeStr.includes(' ')) {
          try {
            ...
            console.warn(
              `[parseQuoteLine] Failed to parse US timestamp: ${timeStr}`,
              e
            );
            // 保留默认的 Date.now()
          }
        ...
        console.warn(
          `[parseQuoteLine] Insufficient data fields for US-share quote: ${codeWithPrefix}`
        );
        return null;
      }
    } else {
      console.warn(
        `[parseQuoteLine] Unknown market type for code: ${codeWithPrefix}`
      );
      return null; // 未知市场类型
    }
    ...
    if (
      quote.code &&
      quote.name &&
      !isNaN(quote.currentPrice!) &&
      !isNaN(quote.changePercent!) &&
      !isNaN(quote.changeAmount!)
    ) {
      return quote as Quote;
    } else {
      console.warn(
        `[parseQuoteLine] Failed to parse quote for ${codeWithPrefix}: Invalid or missing essential data after parsing.`
      );
      return null;
    }
  } catch (parseError) {
    console.error(
      `[parseQuoteLine] Error parsing quote line for ${codeWithPrefix}:`,
      parseError,
      'Line:',
      line
    );
...
export async function fetchQuotes(codes: string[]): Promise<Quote[]> {
  ...
  for (const code of validCodes) {
    ...
    if (cached) {
      ...
      console.log(`[fetchQuotes] 使用缓存数据: ${code}`);
    } else {
  ...
  try {
    ...
    const parsedQuotes: (Quote | null)[] = lines
      .map(parseQuoteLine)
      .filter((q) => q !== null);
    ...
    for (const quote of parsedQuotes) {
      if (quote) {
        const cacheKey = `${QUOTE_CACHE_PREFIX}${quote.code}`;
    ...
    if (axios.isAxiosError(error)) {
      console.error(
        `[fetchQuotes] Axios error fetching quotes:`,
        error.message,
        error.config?.url
      );
      if (error.response) {
...
export async function fetchKline(
  ...
  // 性能优化：添加缓存
  const cacheKey = `${KLINE_CACHE_PREFIX}${code}:${period}:${startDate || ''}:${endDate || ''}:${fq}:${count}`;
  const cachedData = cacheService.get<KlinePoint[]>(cacheKey);
  if (cachedData !== null) {
    console.log(`[fetchKline] 从缓存返回 ${code} 的 K 线数据`);
    return cachedData;
  }
  ...
  if (market === 'us') {
    ...
    if (!knownUSIndices.has(code) && !processedCode.includes('.')) {
      console.log(
        `[fetchKline] Adding .OQ suffix for non-index US code: ${code}`
      );
      processedCode = `${processedCode}.OQ`;
    } else if (knownUSIndices.has(code)) {
      console.log(
        `[fetchKline] Using original code (no suffix) for known US index: ${code}`
      );
  ...
  if (market === 'sh' || market === 'sz') {
    ...
    console.log(
      `[fetchKline] US market requested for ${code}. Forcing path to '/usfqkline/get' and fq to 'qfq' as per verified documentation.`
    );
  ...
  console.log(
    `[fetchKline] Fetching URL: ${fullApiUrl} with params: ${params.param}`
  );
  ...
  while (retryCount <= maxRetries) {
    try {
      ...
      if (Buffer.isBuffer(response.data)) {
        console.log(
          `[fetchKline] Detected Buffer response, attempting to decode for ${code}`
        );
        try {
          // 尝试用iconv-lite解码Buffer
          const decodedStr = iconv.decode(response.data, 'gbk');
          responseData = JSON.parse(decodedStr);
          console.log(
            `[fetchKline] Successfully decoded Buffer response for ${code}`
          );
        } catch (decodeError) {
          console.warn(
            `[fetchKline] Failed to decode Buffer response for ${code}, treating as raw:`,
            decodeError
          );
          // 如果解码失败，尝试其他编码或直接返回空
          return [];
      ...
      // 记录详细响应数据（始终记录以便调试）
      console.log(
        `[fetchKline] Processed response for ${code} (period: ${period}, fq: ${effectiveFq}):`,
      ...
      if (!responseData || typeof responseData !== 'object') {
        console.error(
          `[fetchKline] Invalid response format for ${code}: not an object`
        );
        retryCount++;
        if (retryCount <= maxRetries) {
          console.log(
            `[fetchKline] Retrying (${retryCount}/${maxRetries}) due to invalid response format...`
          );
      ...
      if (responseData.code === undefined || responseData.code === null) {
        console.error(`[fetchKline] Response has no code field for ${code}`);
        retryCount++;
        if (retryCount <= maxRetries) {
          console.log(
            `[fetchKline] Retrying (${retryCount}/${maxRetries}) due to missing code field...`
          );
          await new Promise((resolve) =>
            setTimeout(resolve, 2000 * retryCount)
          );
      ...
      if (responseData.code !== 0) {
        console.error(
          `[fetchKline] Error from Tencent API for ${code}: ${responseData.msg || 'Unknown error'} (code: ${responseData.code})`
        );
        return [];
      }
      ...
      if (!responseData.data) {
        console.error(`[fetchKline] No data field in response for ${code}`);
        return [];
      }
      ...
      if (!dataContainer) {
        console.error(
          `[fetchKline] No data container found for code ${processedCode} in response`
        );
        console.log(
          `[fetchKline] Available codes in response data:`,
      ...
      // 依次尝试所有可能的字段路径
      console.log(
        `[fetchKline] Trying field keys for ${code}: ${possibleFieldKeys.join(', ')}`
      ); // 打印尝试的字段
      for (const fieldKey of possibleFieldKeys) {
        if (Array.isArray(dataContainer[fieldKey])) {
          klineRawData = dataContainer[fieldKey];
          console.log(
            `[fetchKline] Found valid kline data in field '${fieldKey}' for ${code}`
          );
          break; // 找到有效数据，停止尝试
        } else {
          console.log(
            `[fetchKline] Field '${fieldKey}' not found or not an array for ${code}.`
          ); // 打印未找到的字段
        }
      }
      ...
      if (!klineRawData || !Array.isArray(klineRawData)) {
        console.error(
          `[fetchKline] Invalid or empty kline data structure received for ${code}, period ${period}, fq ${fq}`
        );
        console.log(
          `[fetchKline] Tried field keys: ${possibleFieldKeys.join(', ')}`
        );

        // 记录可用的字段
        if (dataContainer) {
          console.log(
            `[fetchKline] Available keys under data.${processedCode}:`,
            Object.keys(dataContainer)
          );
        }
        ...
        if (retryCount <= maxRetries) {
          console.log(
            `[fetchKline] Retrying (${retryCount}/${maxRetries}) after pause...`
          );
          await new Promise((resolve) =>
            setTimeout(resolve, 2000 * retryCount)
          ); // 递增延迟
      ...
      const klinePoints: KlinePoint[] = klineRawData
        .map((item): KlinePoint | null => {
          ...
          if (!item || item.length < 6) {
            console.warn(
              `[fetchKline] Skipping invalid kline data point for ${code}:`,
              item
            );
      ...
      console.log(
        `[fetchKline] Parsed ${klinePoints.length} kline points initially for ${code}.`
      ); // 打印解析数量
      ...
      console.log(
        `[fetchKline] Successfully fetched and parsed ${klinePoints.length} kline points for ${code}, filtered to ${filteredPoints.length} points.`
      );

      // 性能优化：缓存成功获取的数据
      cacheService.set(
        cacheKey,
        filteredPoints as KlinePoint[],
        KLINE_CACHE_TTL
      );
      ...
      if (axios.isAxiosError(error)) {
        console.error(
          `[fetchKline] Axios error fetching kline for ${code}:`,
          error.message,
          error.config?.url
        );
        if (error.response) {
          console.error(
            '[fetchKline] Error response status:',
            error.response.status
          );
          console.error(
            '[fetchKline] Error response data:',
            error.response.data
          ); // K线接口通常返回JSON，直接打印
        } else if (error.request) {
          console.error(
            '[fetchKline] No response received for kline request:',
            error.request
          );
        }
      } else {
        console.error(
          `[fetchKline] Generic error fetching kline for ${code}:`,
          error
        );
      }
  ...
  console.error(
    `[fetchKline EXIT] Unexpected exit after retry loop for ${code}.`
  );
  return [];
}
