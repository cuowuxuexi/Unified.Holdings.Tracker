...
export class CacheValidationService {
  ...
  private async validatePortfoliosCache(): Promise<void> {
    ...
    try {
      // 从缓存获取数据
      const cachedPortfolios = cacheService.get<Portfolio[]>(
        this.PORTFOLIOS_CACHE_KEY
      );

      // 从文件获取数据
      const filePortfolios = dataService.readJsonFile<Portfolio[]>(
        this.PORTFOLIOS_FILE,
        []
      );
      ...
      // 比较数据
      const isValid = this.comparePortfoliosData(
        cachedPortfolios,
        filePortfolios
      );
  ...
  private async validateIndividualPortfolioCaches(): Promise<void> {
    ...
    try {
      ...
      for (const portfolio of filePortfolios) {
        ...
        if (!cachedPortfolio) {
          console.log(
            `[CacheValidation] 投资组合 ${portfolio.id} 缓存不存在，跳过验证`
          );
          continue;
        }
        ...
        if (!isValid) {
          console.log(
            `[CacheValidation] 投资组合 ${portfolio.id} 缓存不一致，刷新缓存`
          );
          cacheService.set(cacheKey, portfolio);
        } else {
          console.log(
            `[CacheValidation] 投资组合 ${portfolio.id} 缓存验证通过`
          );
        }
      }
    } catch (error) {
  ...
  private comparePortfoliosData(
    ...
    for (const [id, cachedPortfolio] of cachedMap) {
      ...
      if (
        !filePortfolio ||
        !this.comparePortfolioData(cachedPortfolio, filePortfolio)
      ) {
        return false;
      }
    }
  ...
  private comparePortfolioData(cached: Portfolio, file: Portfolio): boolean {
    ...
    if (cached.leverage && file.leverage) {
      if (
        cached.leverage.totalAmount !== file.leverage.totalAmount ||
  ...
  public async validateCache(portfolioId?: string): Promise<void> {
    if (portfolioId) {
      ...
      if (portfolio) {
        ...
        if (
          cachedPortfolio &&
          !this.comparePortfolioData(cachedPortfolio, portfolio)
        ) {
          console.log(
            `[CacheValidation] 手动验证：投资组合 ${portfolioId} 缓存不一致，刷新缓存`
          );
          cacheService.set(cacheKey, portfolio);
        }
      }
