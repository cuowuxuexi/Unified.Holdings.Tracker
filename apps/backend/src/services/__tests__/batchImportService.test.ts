jest.mock('../currencyService', () => ({
  ...
  getExchangeRateForAssetToCNY: jest.fn(),
}));
...
>;
const mockedGetExchangeRateForAssetToCNY =
  getExchangeRateForAssetToCNY as jest.MockedFunction<
    typeof getExchangeRateForAssetToCNY
  >;

beforeEach(() => {
  mockedGetExchangeRate.mockReset();
  mockedGetExchangeRateForAssetToCNY.mockReset();
  mockedGetExchangeRate.mockImplementation((from: string, to?: string) => {
    const pair = to ? `${from}-${to}` : from;
    if (pair === 'HKD-CNY') return 0.9;
    if (pair === 'USD-CNY') return 7.2;
    return 1;
  });
  mockedGetExchangeRateForAssetToCNY.mockImplementation(() => 1);
});
  ...
  describe('validateRow', () => {
    ...
    it('无法获取汇率的非CNY交易应报错', () => {
      ...
      expect(
        errors.some(
          (e) =>
            e.field === '货币' && e.message.includes('无法获取 HKD->CNY 的汇率')
        )
      ).toBe(true);
    });
...
2025-01-19,DIVIDEND,sh600519,,,5000,,,CNY,1,分红`;
  ...
  describe('汇率处理', () => {
    it('非CNY现金操作应自动解析汇率并通过预览', async () => {
      mockedGetExchangeRate.mockImplementation((from: string, to?: string) => {
        const pair = to ? `${from}-${to}` : from;
        return pair === 'USD-CNY' ? 7.2 : 1;
      });
      ...
      expect(mockedGetExchangeRate).toHaveBeenCalled();
      expect(preview.rows[0].currency).toBe('USD');
      expect(preview.summary.invalidRows).toBe(0);
      expect(preview.rows[0].exchangeRate).toBeCloseTo(7.2);
    });

    it('无法获取汇率时应在预览阶段提示错误', async () => {
      mockedGetExchangeRate.mockReturnValue(null);
      ...
      expect(preview.summary.invalidRows).toBe(1);
      expect(preview.rows[0].currency).toBe('HKD');
      expect(
        preview.validationErrors.some((e) =>
          e.message.includes('无法获取 HKD->CNY 的汇率')
        )
      ).toBe(true);
    });
  });
