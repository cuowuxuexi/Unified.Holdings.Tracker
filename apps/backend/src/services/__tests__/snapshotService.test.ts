...
describe('snapshotService - P0 snapshotDate kline backfill', () => {
  ...
  it('使用 snapshotDate 的K线收盘价回填，并重算组合汇总', async () => {
    ...
    const updatePositionCall = (prisma.$executeRawUnsafe as jest.Mock).mock.calls.find(
      (args) =>
    ...
    const updatePortfolioCall = (prisma.$executeRawUnsafe as jest.Mock).mock.calls.find(
      (args) =>
        typeof args[0] === 'string' &&
