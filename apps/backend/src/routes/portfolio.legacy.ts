  ...
  asyncHandler(async (req: Request, res: Response, next: NextFunction) => {
    ...
    try {
      ...
      const { name, cash, leverageInfo } = req.body as {
        ...
        leverageInfo?: {
          totalCredit: number;
          interestRate: number /* 其他前端可能发送的字段 */;
        };
      };
...
router.get(
  ...
  asyncHandler(async (req: Request, res: Response, next: NextFunction) => {
    try {
      ...
      console.error(
        'Error initiating historical transaction amount correction:',
        error
      );
  ...
  asyncHandler(async (req: Request, res: Response, next: NextFunction) => {
    ...
    try {
      ...
      if (portfolio.transactions && portfolio.transactions.length > 0) {
        ...
        console.log(
          `[Portfolio Detail Route] 无交易记录，整个生命周期融资成本为 0`
        );
      }
      ...
      console.error(
        `Error fetching portfolio details for ID ${req.params.id}:`,
        error
      );
      next(error); // Use next for error handling
...
router.get(
  ...
  asyncHandler(async (req: Request, res: Response, next: NextFunction) => {
    ...
    try {
      ...
      console.error(
        `Error fetching transactions for portfolio ${req.params.id}:`,
        error
      );
      next(error); // Use next for error handling
...
router.post(
  ...
  asyncHandler(async (req: Request, res: Response, next: NextFunction) => {
    ...
    console.log(
      `[POST /:id/transactions] Request body:`,
      JSON.stringify(req.body, null, 2)
    );
    // --- 结束日志 ---
    try {
      ...
      if (type === TransactionType.BUY || type === TransactionType.SELL) {
        ...
        type === TransactionType.LEVERAGE_ADD ||
        type === TransactionType.LEVERAGE_REMOVE ||
        type === TransactionType.LEVERAGE_COST
      ) {
        ...
        if (amount === undefined || typeof amount !== 'number' || amount <= 0) {
          return res.status(400).json({
            message:
              'Invalid amount for DIVIDEND transaction (must be positive number).',
          });
        }
      ...
      // --- 添加日志：传递给服务层的数据 ---
      console.log(
        `[POST /:id/transactions] Passing data to addTransactionToPortfolio:`,
        JSON.stringify(transactionData, null, 2)
      );
      // --- 结束日志 ---
      const newTransaction = await addTransactionToPortfolio(
        portfolioId,
        transactionData
      );
      // --- 添加日志：服务层返回结果 ---
      console.log(
        `[POST /:id/transactions] addTransactionToPortfolio returned:`,
      ...
      if (!newTransaction) {
        ...
        console.log(
          `[POST /:id/transactions] Service layer returned null (Portfolio or Transaction not found?), returning 404.`
        );
        // --- 结束日志 ---
        return res.status(404).json({ message: 'Portfolio not found' });
      }
      ...
      console.error(
        `Error adding transaction to portfolio ${req.params.id}:`,
        error
      );
      ...
      if (
        ...
        (error as any).statusCode = 400; // Add status code for the error handler
        return next(error);
      }
...
router.delete(
  ...
  asyncHandler(async (req: Request, res: Response, next: NextFunction) => {
    ...
    try {
      ...
      console.error(
        `Error deleting transaction ${req.params.txId} from portfolio ${req.params.id}:`,
        error
      );
      next(error); // Use next for error handling
    ...
    try {
      ...
      const period:
        ...
        rawPeriod && allowedPeriods.includes(rawPeriod)
          ...
          : 'total'; // Default to 'total' if invalid or missing
      ...
      if (portfolio.transactions && portfolio.transactions.length > 0) {
        ...
        leverageCostLifetimeStats = calculateLeverageCostByDay(
          portfolio,
          startDateLifetimeStats,
          endDateLifetimeStats
        );
        console.log(
        ...
        console.log(
          `[Portfolio Stats Route] 无交易记录，整个生命周期融资成本为 0`
        );
      }
      ...
      console.error(
        `Error fetching portfolio stats for ID ${req.params.id}:`,
        error
      );
      next(error);
    }
...
router.get(
  ...
  asyncHandler(async (req: Request, res: Response, next: NextFunction) => {
    try {
      ...
      console.error(
        `Error recalculating cash for portfolio ${req.params.id}:`,
        error
      );
      next(error);
    }
