...
async function initializeDatabase() {
  ...
  try {
    const tableStatements: Record<string, string> = {
      ...
      `,
      YieldCurveSnapshot: `
        CREATE TABLE IF NOT EXISTS "YieldCurveSnapshot" (
          "id" INTEGER NOT NULL PRIMARY KEY AUTOINCREMENT,
          "date" TEXT NOT NULL,
      ...
      MacroIndicatorSnapshot: `
        CREATE TABLE IF NOT EXISTS "MacroIndicatorSnapshot" (
          "id" INTEGER NOT NULL PRIMARY KEY AUTOINCREMENT,
          "date" TEXT NOT NULL,
    ...
    const indexStatements = [
      ...
      `CREATE UNIQUE INDEX IF NOT EXISTS "SourceRun_runKey_key" ON "SourceRun"("runKey")`,
      `CREATE INDEX IF NOT EXISTS "SourceRun_sourceId_startedAt_idx" ON "SourceRun"("sourceId", "startedAt")`,
      `CREATE INDEX IF NOT EXISTS "SourceRun_domain_targetDate_idx" ON "SourceRun"("domain", "targetDate")`,
      `CREATE INDEX IF NOT EXISTS "SourceRun_status_startedAt_idx" ON "SourceRun"("status", "startedAt")`,
      `CREATE UNIQUE INDEX IF NOT EXISTS "SourceHealth_sourceId_domain_key" ON "SourceHealth"("sourceId", "domain")`,
      `CREATE INDEX IF NOT EXISTS "SourceHealth_domain_status_idx" ON "SourceHealth"("domain", "status")`,
      `CREATE INDEX IF NOT EXISTS "SourceHealth_checkedAt_idx" ON "SourceHealth"("checkedAt")`,
      `CREATE UNIQUE INDEX IF NOT EXISTS "YieldCurveSnapshot_date_country_tenor_sourceId_key" ON "YieldCurveSnapshot"("date", "country", "tenor", "sourceId")`,
      `CREATE INDEX IF NOT EXISTS "YieldCurveSnapshot_date_country_idx" ON "YieldCurveSnapshot"("date", "country")`,
      `CREATE INDEX IF NOT EXISTS "YieldCurveSnapshot_country_tenor_date_idx" ON "YieldCurveSnapshot"("country", "tenor", "date")`,
      `CREATE INDEX IF NOT EXISTS "YieldCurveSnapshot_sourceId_date_idx" ON "YieldCurveSnapshot"("sourceId", "date")`,
      `CREATE INDEX IF NOT EXISTS "YieldCurveSnapshot_status_date_idx" ON "YieldCurveSnapshot"("status", "date")`,
      `CREATE UNIQUE INDEX IF NOT EXISTS "MacroIndicatorSnapshot_date_indicatorId_sourceId_key" ON "MacroIndicatorSnapshot"("date", "indicatorId", "sourceId")`,
      `CREATE INDEX IF NOT EXISTS "MacroIndicatorSnapshot_indicatorId_date_idx" ON "MacroIndicatorSnapshot"("indicatorId", "date")`,
      `CREATE INDEX IF NOT EXISTS "MacroIndicatorSnapshot_sourceId_date_idx" ON "MacroIndicatorSnapshot"("sourceId", "date")`,
      `CREATE INDEX IF NOT EXISTS "MacroIndicatorSnapshot_status_date_idx" ON "MacroIndicatorSnapshot"("status", "date")`,
    ];
